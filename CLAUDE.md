...
- `.github/copilot-instructions.md`: GitHub Copilot コードレビュー向け指示ファイル
...
- **pnpm** が必須（preinstall フックの `only-allow pnpm` で強制）
- npm や yarn は使用不可
- バージョンは `package.json` の `packageManager` フィールドで固定（現在 pnpm v11 系）
